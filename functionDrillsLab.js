...
const theEliminator = (contestantsArg, loserArg) => {
  for (i = 0; i < contestantsArg.length; i++) {
    if (contestantsArg[i] === loserArg) {
      contestantsArg.splice(i, 1);
    }
  }
  return contestantsArg;
};
...
const kretzFunction = (str) => {
  console.log(str.toUpperCase());
};

kretzFunction(sampleString);
...
console.log(emailCheck("kretzmer42@gmail.com"));
...
const frogFunc = (goldParam) => {
  return Math.floor(goldParam / 3);
};
let totalFrogs = frogFunc(92.99);
...
// see above, already accounted for
...
let arrayIsAscending = funkyFunc(sampleArray);
